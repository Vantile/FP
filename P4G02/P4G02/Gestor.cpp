...
void inicializar(tGestor &gestor, string dominio) // Inicializa el gestor.
{
...
bool arrancar(tGestor &gestor, string dominio) // Arranca (carga las listas) el gestor.
{
	bool ok = false;
...
bool crearCuenta(tGestor &gestor) // Crea una cuenta nueva en el dominio.
{
	...
	if((gestor.usuarios.contador + 1) < MAX_USUARIOS)
	{
		cout << endl << INTNOM;
		cin.sync();
		string id;
		...
		else
		{
			...
			inicializar(usuarioNuevo, id, contrasenia);
			bool ok1 = insertar(gestor.usuarios, usuarioNuevo);
			if(ok1 == true)
			{
				int activo;
				buscarUsuario(gestor.usuarios, id, activo);
				if(activo != -1)
				{
...
bool iniciarSesion(tGestor &gestor) // Inicia sesion con una cuenta del dominio.
{
	...
	else
	{
		...
		cin.sync();
		cin >> contrasenia;
		bool valido = validarContrasenia(gestor.usuarios.usuarios[posicion], id, contrasenia);
		if(valido == true)
		{
...
{
	system("cls");
	cout << BIENVBANDEJA << gestor.usuarios.usuarios[gestor.usuarioActivo].id << "@" << gestor.dominio << endl;
	if(entrada == true)
	...
	if(entrada == true)
	{
		...
		while((i >= 0) && (ok == true))
		{
			cout << setw(4) << (gestor.usuarios.usuarios[gestor.usuarioActivo].recibidos.contador - i);
			cout << setw(3) << "-";
			int posicion;
	...
	else
	{
		...
		while((i >= 0) && (ok == true))
		{
			cout << setw(4) << (gestor.usuarios.usuarios[gestor.usuarioActivo].enviados.contador - i);
			cout << setw(3) << "-";
			int posicion;
...
void ejecutarMenuBandeja(tGestor &gestor, int opcion, bool &entrada) // Ejecuta la opcion seleccionada del menu de la bandeja.
{
	if(entrada == true) // Bandeja de entrada.
	{
		...
		case 2: // Enviar correo.
			{
				tCorreo correo;
				correoNuevo(correo, (gestor.usuarios.usuarios[gestor.usuarioActivo].id + "@" + gestor.dominio));
...
void menuLeerCorreo(tGestor &gestor, const tCorreo &correoOriginal) // Pinta y ejecuta la opcion del menu de lectura del correo.
{
	...
	case 1: // Contestar correo.
		{
			tCorreo correoNuevo;
			correoContestacion(correoOriginal, correoNuevo, (gestor.usuarios.usuarios[gestor.usuarioActivo].id + "@" + gestor.dominio));
...
void enviarCorreo (tGestor &gestor, const tCorreo &correo) // Envia un correo a otro usuario del sistema.
{
	if((gestor.correos.numCorreos) < MAX_CORREOS)
	{
		if((gestor.usuarios.usuarios[gestor.usuarioActivo].enviados.contador) < MAX_REGISTROS)
		{
			...
			registroRec.leido = 0;
			int posicion = -1;
			int posstr = correo.destinatario.rfind("@");
			string dest = correo.destinatario;
			int num = dest.length();
			dest.erase(posstr, num);
			buscarUsuario(gestor.usuarios, dest, posicion);
