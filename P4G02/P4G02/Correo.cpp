...
void correoContestacion(const tCorreo &correoOriginal, tCorreo &correo, std::string emisor) // Crea un correo respuesta de otro.
{
	...
	cuerpo << correoOriginal.cuerpo << endl << endl << cuerpoEnt.str();
...
string mostrarFecha(tFecha fecha) // Muestra la fecha (fecha y hora) de un correo.
{   
...
string mostrarFechaSoloDia(tFecha fecha) // Muestra la fecha (solo fecha) de un correo.
{
	stringstream resultado;
	tm ltm;
...
string aCadena(const tCorreo &correo) // Traduce a string (para sacar por consola) un correo.
{
	stringstream inform;
...
string obtenerCabecera(const tCorreo &correo) // Obtiene la cabecera (para mostrar en bandeja de entrada) de un correo.
{
